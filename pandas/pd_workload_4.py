...
for label, path in files:
    ...
    print(f"TC1 Pandas: {t1:.6f}s")

    # Test 4.2 — date parsing inside timer to match PySpark
    start = time.perf_counter()
    score = (
        df["Monthly_Fee"].notnull() &
        (pd.to_datetime(df["Signup_Date"], errors="coerce") <= pd.Timestamp("2025-01-01"))
    ).mean()
    t2 = time.perf_counter() - start
    print(f"TC2 Pandas: {t2:.6f}s")
    ...
    score = (
        ...
        (merged["Last_Update"] >= pd.to_datetime(merged["Signup_Date"], errors="coerce"))
    ).mean()
    t3 = time.perf_counter() - start
    print(f"TC3 Pandas: {t3:.6f}s")
